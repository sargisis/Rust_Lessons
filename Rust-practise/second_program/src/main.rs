...
fn main() {
    ...
    let result = bin_search(&arr, 135);

    match result {
        Some((found_value, found_index)) => println!("Found value: {found_value} to {found_index}"),
...
fn bin_search(arr: &[i32], desired_value: i32) -> Option<(i32, usize)>{
    ...
    while low_bound <= upper_bound {
        ...
        let mid  = (upper_bound + low_bound) / 2;
        ...
        match mid_value.cmp(&desired_value) {
            ...
            Ordering::Greater =>  {
                upper_bound = match  mid.checked_sub(1) {
                    Some(result ) => result,
                    _ => return None,
                }
            }
